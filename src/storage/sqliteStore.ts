...
export class SQLiteStore {
  ...
  getAliasesForEntity(entityId: string): Alias[] {
    ...
    return rows.map(r => ({
      ...
      embeddingId: r.embedding_id as string | null,
      sourceEpisodeId: r.source_episode_id as string,
  ...
  /** Return entity IDs reachable in exactly one hop from seedIds (outgoing + incoming edges). */
  getConnectedEntityIds(entityIds: string[], scopeId: string): Set<string> {
    if (entityIds.length === 0) return new Set();
    const ph = entityIds.map(() => '?').join(',');
    const rowsOut = this.db.prepare(`
      SELECT DISTINCT object_entity_id FROM facts
      WHERE subject_entity_id IN (${ph})
        AND object_entity_id IS NOT NULL
        AND scope_id = ?
        AND truth_state != 'hidden'
    `).all(...entityIds, scopeId) as Row[];
    const rowsIn = this.db.prepare(`
      SELECT DISTINCT subject_entity_id FROM facts
    ...
    for (const r of rowsOut) neighbors.add(r.object_entity_id as string);
    for (const r of rowsIn) neighbors.add(r.subject_entity_id as string);
    for (const s of seedSet) neighbors.delete(s);
    return neighbors;
  }

  /**
   * BFS from seedEntityIds up to maxHops hops.
   * Returns a Map<entityId, hopDistance> where seeds have distance 0.
   */
  getEntityGraph(seedEntityIds: string[], scopeId: string, maxHops: number): Map<string, number> {
    const distances = new Map<string, number>();
    if (seedEntityIds.length === 0) return distances;
    for (const id of seedEntityIds) distances.set(id, 0);
    let frontier = [...seedEntityIds];
    for (let hop = 1; hop <= maxHops; hop++) {
      if (frontier.length === 0) break;
      const neighbors = this.getConnectedEntityIds(frontier, scopeId);
  ...
  /** Return current facts whose subject or object is in entityIds. */
  getFactsForEntities(entityIds: string[], scopeId: string, limit = 100): Fact[] {
    ...
    const rows = this.db.prepare(`
      ...
      ORDER BY salience DESC, strength DESC
      LIMIT ?
    `).all(scopeId, ...entityIds, ...entityIds, limit) as Row[];
    return rows.map(r => this.rowToFact(r));
  }
  ...
  setBlock(scopeId: string, key: string, value: string): void {
    ...
    `).run(scopeId, key, value, now, now);
  }

  getBlock(scopeId: string, key: string): string | null {
    const row = this.db.prepare(
      'SELECT value FROM memory_blocks WHERE scope_id = ? AND key = ?',
    ).get(scopeId, key) as Row | undefined;
    return row ? (row.value as string) : null;
  }

  getAllBlocks(scopeId: string): Record<string, string> {
    const rows = this.db.prepare(
  ...
  deleteBlock(scopeId: string, key: string): void {
    this.db.prepare(
